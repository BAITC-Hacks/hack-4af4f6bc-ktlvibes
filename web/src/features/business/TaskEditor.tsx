...
export function TaskEditor({ businessId, taskId, onPublished, onSavedExit }: TaskEditorProps) {
  ...
  useEffect(() => {
    ...
    getTask(taskId)
      .then(async (task) => {
        ...
        const savedRating = task.rating ?? await scorePreview(task.id, savedCard);
        if (!active) return;
  ...
  const askNext = async (currentId: number, currentCard: CardFields) => {
    const result = await getQuestions(currentId, description, currentCard);
  ...
  const saveCard = async (currentId: number, currentCard: CardFields) => {
    const saved = await saveDraft(currentId, currentCard);
  ...
  const makeDraft = () => run("draft", async () => {
    ...
    setRating(await scorePreview(task.id, card));
    setMessage("Черновик создан. Отвечайте на вопросы по одному.");
  ...
  const saveAndExit = () => run("exit", async () => {
    ...
    else await createDraft(description.trim(), card.topic.trim());
    onSavedExit();
  });
